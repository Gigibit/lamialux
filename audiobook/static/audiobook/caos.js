...
console.log("Dispositivo mobile rilevato:", mobileCheck);
...
// Impostazioni di base comuni a entrambi
const baseConfig = {
    CAPTURE_RESOLUTION: 512,
    DENSITY_DISSIPATION: 1,
    VELOCITY_DISSIPATION: 0.2,
...
// Impostazioni specifiche per i cellulari (leggere)
const mobileConfig = {
    SIM_RESOLUTION: 16,          // 📉 Ridotto da 32. La simulazione sarà più "a blocchi".
    DYE_RESOLUTION: 128,         // 📉 Ridotto da 256. Meno pixel da colorare.
    PRESSURE_ITERATIONS: 3,      // 📉 Ridotto da 5.
    CURL: 0,                     // ☢️ AZZERATO. Questa è la modifica CHIAVE.
    SPLAT_RADIUS: 0.5,           // Leggermente più grande per compensare
    SHADING: false,
    BLOOM: false,
    SUNRAYS: false,
    BLOOM_ITERATIONS: 4,         // Ridotto comunque
    BLOOM_RESOLUTION: 128,
    BLOOM_INTENSITY: 0.6,
    BLOOM_THRESHOLD: 0.8,
    BLOOM_SOFT_KNEE: 0.7,
    SUNRAYS: false,              // Disabilitato, impatto notevole
    SUNRAYS_RESOLUTION: 128,
    SUNRAYS_WEIGHT: 1.0,
};

// Impostazioni specifiche per desktop (alta qualità)
const desktopConfig = {
    SIM_RESOLUTION: 128,
    DYE_RESOLUTION: 1024,
    PRESSURE_ITERATIONS: 20,
...
function update () {
    ...
    // --- INIZIO NUOVO CODICE ---
    // Aggiorna il puntatore simulato
    if (!config.PAUSED) {
...
function resizeCanvas() {
    ...
    let width = DEFAULT_DESKTOP_WIDTH;
    let height = DEFAULT_DESKTOP_HEIGHT;
...
function startAutomatedDrag() {
    // Seleziona il primo puntatore disponibile per la simulazione
    let pointer = pointers[0];
    if (pointer == null) return;

    // Imposta lo stato del puntatore automatico
...
function updateAutomatedDrag(dt) {
    ...
    if (!automatedPointer.active && automatedDragTimer >= nextDragTime) {
        ...
        // Imposta il prossimo trascinamento tra 4 e 8 secondi
        nextDragTime = 4.0 + Math.random() * 4.0; 
    }

    // Se non c'è un trascinamento attivo, esci
